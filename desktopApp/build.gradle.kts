...
tasks.named<hydraulic.conveyor.gradle.WriteConveyorConfigTask>("writeConveyorConfig") {
    ...
    doLast {
        destination.get().asFile.appendText(
            """
            |app.fsname = ayushmuzic
            |app.display-name = AyushMuzic
            |app.rdns-name = com.ayush.muzic
            |
...
tasks.register("packageConveyorAppImage") {
    ...
    doLast {
        ...
        desktopFile.writeText(
            ...
            |Version=1.0
            |Name=AyushMuzic
            |Comment=AyushMuzic v$versionName - FOSS YouTube Music Client
            |Exec=bin/simpmusic %u
            |Icon=simpmusic
            |Terminal=false
            |Categories=Audio;AudioVideo;
            |StartupWMClass=AyushMuzic
